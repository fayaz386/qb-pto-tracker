...
app.get("/api/users", requireAdmin, async (req, res) => {
  try {
    const r = await pool.query(`SELECT id, username, role, first_name, last_name, created_at FROM users ORDER BY id ASC`);
...
`;
    ...
    res.json({
      ...
      current_balance: {
        ...
        vacation_used: Number(emp.vacation_used_amount || 0),
        sick_used: Number(emp.sick_used_amount || 0),
        sick_used_hours: Number(emp.sick_used_hours || 0),
        sick_days_used: Number(emp.sick_days_used || 0),
        bereavement_used_hours: Number(emp.bereavement_used_hours || 0),
        bereavement_used_amount: Number(emp.bereavement_used_amount || 0),
        bereavement_days_used: Number(emp.bereavement_days_used || 0)
      },
...
app.put("/api/manual-entry/:id", requireAdmin, async (req, res) => {
  try {
    ...
    if (days !== undefined) { updates.push(`days = $${idx++}`); values.push(days); }
...
app.get("/api/reports/summary-v2", async (req, res) => {
  try {
    ...
    const rows = r.rows.map(row => {
      ...
      const sickHrs = row.sick_used_hours != null ? Number(row.sick_used_hours) : 0;
      const berHrs = row.bereavement_used_hours != null ? Number(row.bereavement_used_hours) : 0;
...
if (fs.existsSync(pfxPath)) {
  ...
  server = https.createServer(options, app).listen(PORT, () => {
    ...
    console.log("NOTE: Access via https://10.0.0.183:8085");
    console.log("SERVER VERSION: v1.4.0 (Secure HTTPS Enabled)");
  });
} else {
